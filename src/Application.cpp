...
Application::Application()
    : m_ads(0x48), m_lastSentTime(g_persistantData.sensorsReadingsUpdateInterval), m_MQ4Sensor(std::nullopt, m_ads, ADSA1, INPUT),
      m_MQ2Sensor(std::nullopt, m_ads, ADSA2, INPUT), m_RCWLSensor(RCWLPIN)
{
    // initling EEPROM
    ...
    m_ControlGPIOsList["washbasineLight"] = std::make_shared<bathRoom::bathRoomGPIO>(WASHBASINELIGHTPIN, bathRoom::GPIOtype::activeLow,
                                                                                     "washbasineLight", WASHBASINELIGHTBUTTONPIN);
    m_ControlGPIOsList["ventilator"] =
        std::make_shared<bathRoom::bathRoomGPIO>(VENTILATORPIN, bathRoom::GPIOtype::activeLow, "ventilator", VENTILATORBUTTONPIN);
...
void Application::checkHazeredSensors()
{
    ...
    if ((millis() - lastReadTimer) > ONESECOND)
    {
        ...
        if ((buzzerState == helper::OFF) && (m_MQ4Sensor.readAnalogValue() >= g_persistantData.MQ4AnalogThrethhold) ||
...
template<typename T> void Application::writeToEEPROM(int address, T xdata)
{
...
template<typename T> void Application::readFromEEPROM(int address, T& xdata)
{
    EEPROM.get(address, xdata);

    Serial.printf("Read from Persistance storage - mq2 threshold: %f, mq4 threshold: %f, MotionEnable: %d\n", xdata.MQ2AnalogThrethhold,
                  xdata.MQ4AnalogThrethhold, xdata.MotionEnable);
}
