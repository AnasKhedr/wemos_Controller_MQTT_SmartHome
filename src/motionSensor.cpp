...
// RCLWSensor::RCLWSensor(const uint8_t& sensorStatePin, bathRoom::bathRoomGPIO&
// lightControlPin) :
RCLWSensor::RCLWSensor(const uint8_t& sensorStatePin)
    : m_sensorStatePin(sensorStatePin),
      // m_lightControlPin(lightControlPin),
      m_isEnabled(true), didMotionSensorTurnOnLight(false)
{
...
void RCLWSensor::controlLight()
{
    if (m_isEnabled)
    {
        ...
        else
        {
            ...
            if (didMotionSensorTurnOnLight && ((millis() - m_motionDetectedTime) > g_persistantData.motionSensorLightActiveTime))
            {
