...
public:
    ...
    //!
    officeGPIO(const uint8_t controlPin, const GPIOtype type, const std::string topic = "NULL",
    ...
    //! \param mqttClients
    //!
    void checkButton(const std::vector<std::unique_ptr<mqtt::mqttClient>>& mqttClients);
    // if we don't have any mqtt clients
    void checkButton();
...
private:
    ...
    //!
    inline static std::vector<uint8_t> alreadyUsedGPIOs;

    //---------------------------------------------------------------------------
    //! \brief a list of all the valid pins on the ESP board.
    //!
    inline static std::vector<uint8_t> validPins {D0, D1, D2, D3, D4, D5, D6, D7, D8, D9, D10};
};
