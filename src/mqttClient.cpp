...
namespace mqtt
{
mqttClient::mqttClient(std::string ip, std::string subscriptionTopic)
    : m_brokerIp(ip), isBrokerInitialized(false), m_subscriptionTopic(subscriptionTopic)
{
...
// mqttClient::mqttClient(std::string ip, std::string subscriptionTopic, onMessageCallbackType callback) :
...
std::string mqttClient::getBrokerIp()
{
    return m_brokerIp;
}
...
bool mqttClient::init()
{
    ...
    if (!m_mqttClient.connected())
    {
        ...
        {
            yield();
            _PRINTF("[mqttClient] failed to connect to port %d\n", MQTTPORT);
            m_mqttClient.connect();
            returnCode = m_mqttClient.connected();
            // _PRINT("failed, rc= ");
    ...
    {
        _PRINTF("[mqttClient] Already connected to MQTT broker: %s on port: %d , doing nothing\n", m_brokerIp.c_str(), MQTTPORT);
    }
...
bool mqttClient::loop()
{
    ...
    if (!m_mqttClient.connected())
    {
        // _PRINTF("connection state to: %s  is: %d\n", m_brokerIp.c_str(), m_mqttClient.connected());
            ...
            else
            {
                ...
                else
                {
                    ...
                    _PRINTF("Failed to reconnect retry #: %d, retrying again in %d seconds.\n", numberOfRetries,
                            MQTTRECONNECTIONINTERVALS / 1000);
                }
            }
        }
    }
    ...
    {
        // _PRINTF("already connected to mqtt broker %s\n", m_brokerIp.c_str());
        returnValue = true;
...
void mqttClient::addObserver(IObserver& observer)
{
...
void mqttClient::onMqttConnect(bool sessionPresent)
{
    ...
    uint16_t packetIdSub = m_mqttClient.subscribe(m_subscriptionTopic.c_str(), 2);
    _PRINT("Subscribing at QoS 0, packetId: ");
    _PRINTLN(packetIdSub);
    m_mqttClient.publish("/home/office/", 0, false, "connected");
}

void mqttClient::onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index,
                               size_t total)
{
    std::string message(payload, len);
    helper::printMqttMessage(topic, message);

    for (auto& oneObserver : m_observers)
    {
        // notify all the subscribers. e.g. Application::onMqttMessage
        oneObserver.get().onMqttMessage(topic, message);
    }
