...
bathRoomGPIO::bathRoomGPIO(const uint8_t controlPin, const GPIOtype type, const std::string handle,
                           const std::optional<uint8_t> toggelButton, const uint8_t internalResistor)
    : m_controlPin(controlPin), m_toggelButton(toggelButton), m_type(type)
...
void bathRoomGPIO::checkButton(std::vector<std::shared_ptr<mqtt::mqttClient>>& mqttClients)
{
    ...
    if (!m_toggelButton)
    {
        // Serial.printf("device %s connected to pin: %d, with type m_type: %d, has not toggle
