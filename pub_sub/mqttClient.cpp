...
namespace mqtt
{
    ...
    bool mqttClient::loop()
    {
        ...
        if (!m_mqttClient.connected())
        {
            _PRINTF("connection state to: %s  is: false\n", m_brokerIp.c_str());
            ...
            if ((now - m_lastReconnectAttempt) > MQTTRECONNECTIONINTERVALS)
            {
                ...
                else
                {
                    ...
                    {
                        now = millis();
                        _PRINTF("Failed to reconnect retry #: %d on port: %d, retrying again in %d seconds.\n", numberOfRetries, MQTTPORT, MQTTRECONNECTIONINTERVALS/1000);
                    }
                }
            }
        }
    ...
    {
        _PRINTF("Disconnected from MQTT, reason: %d\n", static_cast<int>(reason));
        m_isConnected = false;
    }
